...
import { generateAccessToken, generateRefreshToken, options, verifyRefreshToken } from "../utils/jwt.util.js";
const prisma=new PrismaClient()

const registerCollege=asyncHandler(async(req,res)=>{
  const {name,address,email,phone,password,confirmPassword}=req.body
  if(!name || !address || !email || !password || !confirmPassword) throw new ApiError(403,"provide all fields")
  
	if(password.toUpperCase()!==confirmPassword.toUpperCase()) throw new ApiError(403,"password and confirm password must be same")
...
const registerCompany=asyncHandler(async(req,res)=>{
	...
	let exists = await prisma.company.findFirst({
		where: {
			OR: [{ name: name }, { email: email },{registrationNo:registrationNo}],
		},
	});
...
const refreshController=asyncHandler(async(req,res)=>{
	...
	if(incomingRefreshToken!==user.refreshToken) throw new ApiError(403,"refresh token mismatch")
	...
	res
		...
		.cookie("refreshToken", refreshToken, options)
		.json(
		new ApiResponse(200, {}, "refreshed token successfully")
		);
})
